...
% Outputs frequency list relative to bin spacing.

%#eml

function peakList = curve3FindPeakOffsets(fftSize,spectra,maxFreq)
    ...
    for i=2:fftSize, %Cycle through frequency bins in setFreqMax1 (ignore start to consider two rows. Never a frequency in bin 1=0Hz)
        if(maxFreq(i)==1), % if a peak was identified...
            % increment peak counter
            tempPosition = tempPosition + 1;
            % find peak position in units of bin spacing
            peakList1(tempPosition) = i + peakPolyOffset(spectra(i-1),spectra(i),spectra(i+1));
        end
    end
    if (tempPosition ~= 0),
